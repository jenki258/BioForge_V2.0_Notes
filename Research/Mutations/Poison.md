# Poison

#BioForge #BioForge_V0_54T #Mutation #Rarity_uncommon #Pathogen_BACTERIA #Pathogen_PARASITE #Pathogen_FUNGI #Pathogen_UNIVERSAL #PlayerGuide #ModpackGuide #AddonDevelopment

> Mutation ID: `poison`. Source: `data/bioforge/mutations/legacy_v1_catalog.json`.

## Description

Toxic metabolites periodically poison the host

## Gameplay behavior

- Apply `minecraft:poison` level 1 for 100 ticks every 100 ticks on continuous.

## Compatibility and selection

- Compatible pathogens: BACTERIA, PARASITE, FUNGI, UNIVERSAL.
- Rarity: uncommon; random-selection weight: 30.
- No mutation prerequisite.
- Conflicts with: [[Poison Resistance]], [[Honey Immunity]].
- Behavior tags: `legacy_v1`, `symptom`, `toxin`.

## Interactions

- No explicit cross-mutation interaction is declared in this definition.

## Pack and addon notes

- Enabled definitions participate in compatible weighted selection after requirements and conflicts are checked.
- Keep the ID stable once worlds have saved it. Replace/tune the JSON through a datapack or register new executable effect behavior in Java.

Related: [[Mutation System]], [[Mutation Interactions]], [[Mutation API]].

